...
func (s *Session) NewBatch(typ BatchType) *Batch {
	...
	batch := &Batch{Type: typ, rt: s.cfg.RetryPolicy, serialCons: s.cfg.SerialConsistency,
		Cons: s.cons, defaultTimestamp: s.cfg.DefaultTimestamp, session: s}
	s.mu.RUnlock()
	return batch
}
...
func (b *Batch) GetRoutingKey() ([]byte, error) {
	if b.session != nil && len(b.Entries) > 0 {
		var qry *Query
		s := b.session
		qry = &Query{stmt: b.Entries[0].Stmt, values: b.Entries[0].Args, cons: s.cons,
			session: s, pageSize: s.pageSize, trace: s.trace,
			prefetch: s.prefetch, rt: s.cfg.RetryPolicy, serialCons: s.cfg.SerialConsistency,
			defaultTimestamp: s.cfg.DefaultTimestamp,
		}
		return qry.GetRoutingKey()
	}
	return nil, nil
}
